...
* 📊 **[Справочник по акциям](docs/shares.md)** — (в разработке) Дивиденды, лотность и капитализация.
* ⚙️ **[Справочник по индексам](docs/index.md)** — (в разработке) Описание индексов фондового рынка 
...
Установка зависимостей с помощью [uv](https://github.com/astral-sh/uv):
Стандартная версия:
```bash
uv add https://github.com/Akxios/pymoex.git
```
Версия с бета-функциями
```bash
uv add https://github.com/Akxios/pymoex.git@dev
```
...
- open_price, high_price, low_price: Цены открытия, макс, мин.
- volume_today: Объем торгов (шт).
- value_today: Объем торгов (руб).
...
- sec_id: Тикер.
- short_name: Краткое название.
- last_price: Чистая цена (% от номинала).
